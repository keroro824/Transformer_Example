...
class MultiheadAttention(torch.nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, share_qk=False):
...
#             xavier_uniform_(self.in_proj_weight.weight)
#         else:
#             xavier_uniform_(self.q_proj_weight.weight)
#             xavier_uniform_(self.k_proj_weight.weight)
#             xavier_uniform_(self.v_proj_weight.weight)

#         if self.add_bias_kv is not None:
...
#             xavier_normal_(self.bias_v)    
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None):
        ...
        return self.multihead_attention(query, key, value, attn_mask, self.bias_k, self.bias_v)
...
class MultiheadAttentionContainer(torch.nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        attn_output, self.attn_output_weights = self.attention_layer(q, k, v, attn_mask=attn_mask,
...
class ScaledDotProduct(torch.nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        attn_output_weights_before_dropout = torch.nn.functional.softmax(attn_output_weights, dim=-1)
        attn_output_weights = torch.nn.functional.dropout(attn_output_weights_before_dropout, p=self.dropout, training=self.training)
        attn_output = torch.matmul(attn_output_weights, value)
        return attn_output.transpose(-2, -3), attn_output_weights_before_dropout
